...
/datum/signal
	...
	proc/tcombroadcast(message, freq, source, job, spans, say = "says", ask = "asks", yell = "yells", exclaim = "exclaims")
		...
		virt.name = source
		virt.job = job
		virt.languages_spoken = HUMAN //this has to be changed to somehow take languages from the $language var
		virt.languages_understood = HUMAN //do not forget this or everything turns to jibberish
