...
/datum/map_template/proc/load_new_z()
	...
	//Port lighting changes //SSlighting.initialize_lighting_objects(block(locate(bounds[MAP_MINX], bounds[MAP_MINY], bounds[MAP_MINZ]),locate(bounds[MAP_MAXX], bounds[MAP_MAXY], bounds[MAP_MAXZ])))
...
/datum/map_template/proc/load(turf/T, centered = FALSE)
	...
	var/list/bounds = maploader.load_map(file(mappath), T.x, T.y, T.z, cropMap=TRUE, no_changeturf=/*(SSatoms.initialized == INITIALIZATION_INSSATOMS)*/1) // port SSinitialize
	if(!bounds)
		return
