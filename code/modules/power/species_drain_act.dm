/datum/species/proc/species_drain_act(mob/living/carbon/human/H, atom/A)
	if(!istype(H) || !A)
		return 0

	if(H.get_active_hand() && H.get_active_hand() != A)
		return 0
	...
	while(!done)
		...
		if(H.nutrition + nutritionIncrease > NUTRITION_LEVEL_WELL_FED)
			nutritionIncrease = max(NUTRITION_LEVEL_WELL_FED - H.nutrition, 0) //if their nutrition goes up from some other source, this could be negative, which would cause bad things to happen.
...
//CELL//
/obj/item/weapon/stock_parts/cell/can_consume_power_from()
	if(charge < MIN_DRAINABLE_POWER)
		return "<span class='info'>Power cell depleted, CONSUME protocol halted.</span>"
	return 1

/obj/item/weapon/stock_parts/cell/consume_power_from(amount)
