	...
	for(var/mob/living/carbon/C in targets)
		if(C.mind && user.mind)
			if(C.stat == DEAD)
				...
				var/contractTypeName = input(user, "What type of contract?") as anything in list ("Power", "Wealth", "Prestige", "Magic", "Knowledge")
...
/mob/living/proc/infernalphasein()
	...
	loc = get_turf(src)
	client.eye = src
	visible_message("<span class='warning'><B>[src] appears in a firey blaze!</B>")
