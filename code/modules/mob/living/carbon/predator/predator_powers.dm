...
/mob/living/carbon/human/predator/verb/remove_skull()
	...
	src.visible_message("<span class='danger'>[src] has plunged their sharp, dagger-like fingers into [ptarget]'s eyes!</span>", \
		"<span class='danger'>[src] has plunged their sharp, dagger-like fingers into [ptarget]'s eyes!</span>")
	if(!iszombie(ptarget))
	...
	if(do_after(src, 200, target = ptarget))
		ptarget.visible_message("<span class='danger'>You can feel fingers wrapping around the inside of your skull!</span>")
		src << "<span class='danger'>Now it's time to collect your trophy...</span>"
		ptarget.Weaken(10)
		if(do_after(src, 50, target = ptarget))
			if(ptarget.stat != DEAD)
				ptarget.visible_message("<span class='danger'>In an instant, after feeling all sorts of tremendous pain you are left with a feeling of numbness as your skull is torn from your head.</span>")
				ptarget.death()
