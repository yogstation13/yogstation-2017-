...
/obj/item/weapon/storage/firstaid/attackby(var/obj/item/device/assembly/bikehorn/S, mob/user as mob)
	...
	if(src.contents.len >= 1)
		user << "<span class='notice'>You need to empty [src] out first if you want to attach the bike horn.</span>"
		..()
		return
	...
	qdel(S)
	user.put_in_hands(A)
	user << "<span class='notice'>You crudely attach the bikehorn to the side of [src].</span>"
	user.unEquip(src, 1)
	qdel(src)
...
/obj/item/weapon/honkbox/attackby(var/obj/item/clothing/mask/gas/clown_hat/W, mob/user as mob)
	...
	qdel(W)
	user.put_in_hands(A)
	user << "<span class='notice'>You crudely staple the mask to the top of [src].</span>"
	user.unEquip(src, 1)
	qdel(src)
...
/mob/living/simple_animal/hostile/honkbot/proc/Emag(mob/user as mob)
	if(user) user << "<span class='notice'>[src] honks excitedly!</span>"
	emagged = 2
