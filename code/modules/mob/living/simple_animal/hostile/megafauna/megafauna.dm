...
/mob/living/simple_animal/hostile/megafauna/Life()
	...
	if(loc && (loc.z != ZLEVEL_LAVALAND) && alert_admins_z_level)
		message_admins("A live [name] has left the lavaland and is currently on another z level. <A HREF='?_src_=holder;adminplayerobservefollow=\ref[src]'>FLW</A> ([loc.x], [loc.y], [loc.z])")
	...
	if(!istype(A, /area/lavaland/surface/outdoors) && alert_admins_area)
		message_admins("A live [name] was led [aggroed_from ? "a distance of [get_dist(aggroed_from, src)] tiles " : ""] to [A] by [lastTarget]. Check the aggro_log var of this creature to see where it was led from.<A HREF='?_src_=holder;adminplayerobservefollow=\ref[src]'>FLW</A> ([loc.x], [loc.y], [loc.z])")
