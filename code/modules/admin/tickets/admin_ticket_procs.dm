...
/datum/admin_ticket/proc/add_log(var/message, var/user_in)
	...
	if(!compare_ckey(handling_admin, user))
		if(!(get_ckey(handling_admin) in messageSentTo))
			...
			to_chat(handling_admin, "<span class='ticket-text-received'>-- [get_view_link(user)] [key_name_params(user, 1, 1, null, src)] -> [log_item.isAdminComment() ? get_view_link(user) : key_name_params(handling_admin, 0, 1, null, src)]: [log_item.unfuck_apostrophes()]</span>")
			if(has_pref(handling_admin, SOUND_ADMINHELP))
				to_chat(handling_admin, nullify_sound('sound/effects/adminhelp.ogg'))
		...
		if(!(get_ckey(owner) in messageSentTo))
			...
			if(!compare_ckey(owner_ckey, user))
				...
				if(has_pref(owner, SOUND_ADMINHELP))
					to_chat(owner, nullify_sound('sound/effects/adminhelp.ogg'))
			...
			if(compare_ckey(owner_ckey, user))
				...
				to_chat(owner, "<span class='ticket-text-sent'>-- [key_name_params(owner, 0, 0, null, src)] -> [toLink]: [log_item.unfuck_apostrophes()]</span>")
			else
				to_chat(owner, "<span class='ticket-text-received'>-- [is_admin(owner) ? key_name_params(user, 1, 1, null, src) : key_name_params(user, 1, 0, null, src)] -> [key_name_params(owner, 0, 0, null, src)]: [log_item.unfuck_apostrophes()]</span>")
	...
	if(!compare_ckey(user, owner_ckey))
		if(!(get_ckey(user) in messageSentTo))
			...
			to_chat(user, "<span class='ticket-text-sent'>-- [is_admin(user) ? key_name_params(user, 0, 1, null, src) : "[key_name_params(user, 0, 0, null, src)]"] -> [log_item.isAdminComment() ? get_view_link(user) : (is_admin(user) ? key_name_params(owner, 1, 1, null, src) : "[key_name_params(owner, 1, 0, null, src)]")]: [log_item.unfuck_apostrophes()] [is_admin(user) ? "(<a href='?src=\ref[src];user=\ref[usr];action=resolve_admin_ticket;ticket=\ref[src]'>Close</a>)" : ""]</span>")
	...
	for(var/M in monitors)
		...
		if(compare_ckey(user, owner))
			to_chat(M, "<span class='ticket-text-sent'>-- [get_view_link(user)] [key_name_params(user, 1, 1, null, src)] -> [key_name_params(owner, 0, 1, null, src)]: [log_item.unfuck_apostrophes_admin()]</span>")
		else
			to_chat(M, "<span class='ticket-text-received'>-- [get_view_link(user)] [key_name_params(user, 1, 1, null, src)] -> [key_name_params(handling_admin, 0, 1, null, src)]: [log_item.unfuck_apostrophes_admin()]</span>")

		if(has_pref(M, SOUND_ADMINHELP))
			to_chat(M, nullify_sound('sound/effects/adminhelp.ogg'))
	...
	for(var/client/X in admins)
		if(has_pref(X, TICKET_ALL))
			...
			to_chat(X, "<span class='ticket-text-[(compare_ckey(X, user) || compare_ckey(X, handling_admin) || target == "Admins") ? "received" : "sent"]'>-- [get_view_link(user)] [key_name_params(user, 1, 1)] -> [target]: [log_item.unfuck_apostrophes_admin()]</span>")

	if(compare_ckey(log_item.user, owner_ckey))
		log_admin("Ticket #[ticket_id]: [log_item.user] -> [handling_admin ? handling_admin : "Ticket"] - [log_item.unfuck_apostrophes()]")
	else if(compare_ckey(log_item.user, handling_admin))
		log_admin("Ticket #[ticket_id]: [log_item.user] -> [owner_ckey] - [log_item.unfuck_apostrophes()]")
	else
		log_admin("Ticket #[ticket_id]: [log_item.user] -> Ticket #[ticket_id] - [log_item.unfuck_apostrophes()]")
		...
		if(!handling_admin && !resolved)
			...
			for(var/client/X in admins)
				...
				if(has_pref(X, SOUND_ADMINHELP))
					to_chat(X, nullify_sound('sound/effects/adminhelp.ogg'))
...
/datum/admin_ticket/proc/toggle_resolved()
	...
	for(var/client/C in to_process)
		to_chat(C, "<span class='ticket-status'>-- [get_view_link(C)] has been set to '<b>[resolved ? "resolved" : "unresolved"]</b>' by [key_name_params(usr, is_admin(C), is_admin(C))]</span>")
