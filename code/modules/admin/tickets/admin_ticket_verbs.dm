...
/client/verb/admin_ticket(ticket_title as text)
	...
	if(!found_ticket)
		...
		if(!total_admins_active())
			send_discord_message(DISCORD_ADMIN, "A new ticket has been created with no active admins online, There are now a total of [total_unresolved_tickets()] unresolved tickets.")
