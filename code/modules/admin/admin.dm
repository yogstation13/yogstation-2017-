...
/datum/admins/proc/access_news_network() //MARKER
	...
	switch(admincaster_screen)
		if(0)
			...
			dat+="<BR><HR><A href='?src=\ref[src];ac_set_signature=1'>The newscaster recognises you as:<BR> <FONT COLOR='green'>[admin_signature]</FONT></A>"
		if(1)
		...
		if(2)
			...
			dat+="<HR><B><A href='?src=\ref[src];ac_set_channel_name=1'>Channel Name</A>:</B> [admincaster_feed_channel.channel_name]<BR>"
			dat+="<B><A href='?src=\ref[src];ac_set_signature=1'>Channel Author</A>:</B> <FONT COLOR='green'>[admin_signature]</FONT><BR>"
			dat+="<B><A href='?src=\ref[src];ac_set_channel_lock=1'>Will Accept Public Feeds</A>:</B> [(admincaster_feed_channel.locked) ? ("NO") : ("YES")]<BR><BR>"
			dat+="<BR><A href='?src=\ref[src];ac_submit_new_channel=1'>Submit</A><BR><BR><A href='?src=\ref[src];ac_setScreen=[0]'>Cancel</A><BR>"
		if(3)
			dat+="Creating new Feed Message..."
			dat+="<HR><B><A href='?src=\ref[src];ac_set_channel_receiving=1'>Receiving Channel</A>:</B> [admincaster_feed_channel.channel_name]<BR>" //MARK
			dat+="<B>Message Author:</B> <FONT COLOR='green'>[admin_signature]</FONT><BR>"
			dat+="<B><A href='?src=\ref[src];ac_set_new_message=1'>Message Body</A>:</B> [admincaster_feed_message.returnBody(-1)] <BR>"
		...
		if(12)
			...
			dat+="<FONT SIZE=2><A href='?src=\ref[src];ac_censor_channel_author=\ref[admincaster_feed_channel]'>[(admincaster_feed_channel.authorCensor) ? ("Undo Author censorship") : ("Censor channel Author")]</A></FONT><HR>"
		...
		if(13)
			dat+="<B>[admincaster_feed_channel.channel_name]: </B><FONT SIZE=1>\[ created by: <FONT COLOR='maroon'>[admincaster_feed_channel.returnAuthor(-1)]</FONT> \]</FONT><BR>"
			dat+="Channel messages listed below. If you deem them dangerous to the station, you can <A href='?src=\ref[src];ac_toggle_d_notice=\ref[admincaster_feed_channel]'>Bestow a D-Notice upon the channel</A>.<HR>"
			if(admincaster_feed_channel.censored)
				dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the station, and marked with a Nanotrasen D-Notice.<BR>"
				dat+="No further feed story additions are allowed while the D-Notice is in effect.</FONT><BR><BR>"
			else
		...
		if(14)
			...
			dat+="<HR>"
			dat+="<A href='?src=\ref[src];ac_set_wanted_name=1'>Criminal Name</A>: [admincaster_wanted_message.criminal] <BR>"
			dat+="<A href='?src=\ref[src];ac_set_wanted_desc=1'>Description</A>: [admincaster_wanted_message.body] <BR>"
			if(wanted_already)
