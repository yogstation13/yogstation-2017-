...
/client/proc/check_randomizer(topic)
	...
	if (oldcid)
		if (!topic || !topic["token"] || !tokens[ckey] || topic["token"] != tokens[ckey])
			...
			sleep(150) //browse is queued, we don't want them to disconnect before getting the browse() command.
		...
		if (computer_id != lastcid)
			...
			sleep(50) //browse is queued, we don't want them to disconnect before getting the browse() command.
...
/client/proc/cid_check_reconnect()
	...
	to_chat("<a href='byond://[url]?token=[token]'>You will be automatically taken to the game, if not, click here to be taken manually</a>")
