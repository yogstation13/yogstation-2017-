...
/turf/closed/wall/facility
	name = "facility wall"
	desc = "acid proof perimeter walls."
	icon = 'icons/turf/walls/reinforced_wall.dmi'
	icon_state = "r_wall"
	opacity = 1
	density = 1
...
/turf/closed/wall/facility/break_wall()
	builtin_sheet.loc = src
	return (new /obj/structure/girder/reinforced(src))
...
/turf/closed/wall/facility/attack_animal(mob/living/simple_animal/M)
	M.changeNext_move(CLICK_CD_MELEE)
	M.do_attack_animation(src)
	if(M.environment_smash == 3)
		dismantle_wall(1)
...
/turf/closed/wall/facility/try_destroy(obj/item/weapon/W, mob/user, turf/T)
	if(istype(W, /obj/item/weapon/pickaxe/drill/jackhammer))
		var/obj/item/weapon/pickaxe/drill/jackhammer/D = W
		to_chat(user, "<span class='notice'>You begin to smash though the [name]...</span>")
		if(do_after(user, 50, target = src))
			if( !istype(src, /turf/closed/wall/facility) || !user || !W || !T )
				return 1
			if( user.loc == T && user.get_active_hand() == W )
				D.playDigSound()
				visible_message("<span class='warning'>[user] smashes through the [name] with the [D.name]!</span>", "<span class='italics'>You hear the grinding of metal.</span>")
...
/turf/closed/wall/facility/try_decon(obj/item/weapon/W, mob/user, turf/T)
	//DECONSTRUCTION
	switch(d_state)
		if(0)
			if (istype(W, /obj/item/weapon/wirecutters))
		...
		if(2)
			if( istype(W, /obj/item/weapon/weldingtool) )
				...
				if( WT.remove_fuel(0,user) )
					...
					if(do_after(user, 60, target = src))
						if( !istype(src, /turf/closed/wall/facility) || !user || !WT || !WT.isOn() || !T )
							return 0
...
/turf/closed/wall/facility/proc/update_icon()
	if(d_state)
		icon_state = "r_wall-[d_state]"
		smooth = SMOOTH_FALSE
		clear_smooth_overlays()
...
/turf/closed/wall/facility/singularity_pull(S, current_size)
	if(current_size >= STAGE_FIVE)
		if(prob(30))
			dismantle_wall()