...
// ** Welcome to micropurchase hell** //

//SOURCE: Acrade.dm, massive copypaste inbound//
...
//Handles giving out prizes//


/obj/item/toy/gamechild
	name = "Game Child"
	desc = "That's not a slot for games, that's a slot for your valuables."
	icon_state = "gamechild"
	var/enemy_name = "Space Villian"
	var/temp = "DLC not installed" //Temporary message, for attack messages, etc
	...
	var/turtle = 0
	var/emagged = 0
//Defining what it can drop, as in arcade.dm//
...
/obj/item/toy/gamechild/proc/prizevend()
...
//FEED IT YOUR MONEY SLAVE//

/obj/item/toy/gamechild/attackby(obj/item/I, mob/user, params)

	if(istype(I, /obj/item/stack/sheet/mineral))
		var/obj/item/stack/sheet/mineral/space_moneys = I
		space_moneys.get_amount()
		user << "<span class='notice'>You insert the [I] into the [src], a small dollar sign flashes up on the screen as your money goes down the drain.</span>"
		qdel(I)
		playsound(src.loc, 'sound/machines/ping.ogg', 50, 0)
		player_hp += space_moneys.get_amount()
		player_mp += space_moneys.get_amount()
        //testdis//
	else
		user << "<span class='notice'>Error this machine only accepts mineral sheets, you have to pay big to WIN big!</span>"
		return

/obj/item/toy/gamechild/New()
	..()
	//var/name_action
	var/name_part1
	var/name_part2

	//name_action = pick("Purchase ", "Acquire ", "Download ", "Strike ", "Stop ", "Destroy ", "Robust ", "Romance ", "Pwn ", "Own ", "Ban ")

	name_part1 = pick("the Automatic ", "the free to play ", "Lord ", "Professor ", "the expansion packed ", "the Evil ", "the Dread King ", "the Space ", "Lord ", "the Great ", "Aleksander the ", "General ")
	name_part2 = pick("candy crusher", "clown clasher", "bubblepopper", "panda", "Jeff", "witch", "Crushulon", "Uhangoid", "Vhakoid", "Peteoid", "catbeast", "Griefer", "ERPer", "Lizard Man", "Unicorn", "Bloopers")

	src.enemy_name = replacetext((name_part1 + name_part2), "the ", "")


/obj/item/toy/gamechild/attack_self(mob/user)
	if(..())
		return
	user.set_machine(src)
	...
	//onclose(user, "arcade")
	var/datum/browser/popup = new(user, "arcade", "Micropurchase ")
	popup.set_content(dat)
	popup.set_title_image(user.browse_rsc_icon(src.icon, src.icon_state))
	popup.open()
	return

/obj/item/toy/gamechild/Topic(href, href_list)
	if(..())
		return
...
/obj/item/toy/gamechild/proc/arcade_action()
	if ((src.enemy_mp <= 0) || (src.enemy_hp <= 0))
		if(!gameover)
			...
			if(emagged)
				...
				new /obj/item/stack/spacecash/c1000(src.loc)
				new /obj/item/weapon/twohanded/pitchfork(src.loc)
				message_admins("[key_name_admin(usr)] has beaten the gamechild and won money and a pitchfork.")
				log_game("[key_name(usr)] has beaten the gamechild and won money and a pitchfork.")
				src.New()
				emagged = 0
			else
		...
		if (src.player_mp <= 0)
			src.gameover = 1
			sleep(10)
			src.temp = "Your wallet has been drained! GAME OVER"
	...
	if ((src.player_mp <= 0) || (src.player_hp <= 0))
		src.gameover = 1
		src.temp = "Your wallet is empty! GAME OVER"
		playsound(src.loc, 'sound/arcade/Lose.ogg', 50, 1, extrarange = -3, falloff = 10)
		if(emagged)
...
/obj/item/toy/gamechild/emag_act(mob/user)
	if(!emagged)
		temp = "You'll either be extremely rich, or extremely dead."
		player_hp = 30
		player_mp = 10
		enemy_hp = 45
		enemy_mp = 20
