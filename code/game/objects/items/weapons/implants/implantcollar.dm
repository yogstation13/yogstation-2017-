...
/obj/item/weapon/implant/bombcollar/activate(mob/user as mob)
	switch(alert("Select an option.","Bomb Collar Control","Status","Bind","Exit"))
		if("Status")
			to_chat(imp_in, "<span class='notice'><b>Bomb Collar Status Report:</b></span>")
			for(var/obj/item/clothing/head/bombCollar/C in linkedCollars)
				var/turf/T = get_turf(C)
				to_chat(imp_in, "<b>[C]:</b> [iscarbon(C.loc) ? "Worn by [C.loc], " : ""][get_area(C)], [T.loc.x], [T.loc.y], [C.locked ? "<span class='boldannounce'>Locked</span>" : "<font color='green'><b>Unlocked</b></font>"]")
			return
		if("Bind")
			...
			if(!iscarbon(collarToBind.loc))
				to_chat(imp_in, "<span class='warning'>That collar isn't being held or worn by anyone.</span>")
				return
			var/mob/living/carbon/C = collarToBind.loc
			if(C.head != collarToBind)
				to_chat(imp_in, "<span class='warning'>That collar isn't around someone's neck.</span>")
				return
			for(var/obj/item/clothing/head/bombCollar in boundCollars)
				boundCollars -= collarToBind
				to_chat(imp_in, "You unbind [collarToBind] to you, once you die, they wont.")
				message_admins("[imp_in] unbound [collarToBind]")
				return

			boundCollars += collarToBind
			to_chat(imp_in, "You bind [collarToBind] to you, once you die, they die with you.")
