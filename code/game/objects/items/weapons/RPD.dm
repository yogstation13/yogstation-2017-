...
/datum/pipe_info/disposal/New(var/pid,var/dt)
	id=pid
	icon_state=disposalpipeID2State[pid+1]
	dir=2
	dirtype=dt
	...
	switch(p_class)
		if(PAINT_MODE) // Paint pipes
			if(!istype(A,/obj/machinery/atmospherics/pipe))
				...
				to_chat(user, "<span class='warning'>\The [abbreviated_name]'s error light flickers!  Perhaps you need to only use it on pipes and pipe meters?</span>")
				return 0
		...
		if(ATMOS_MODE)
			if(!(istype(A, /turf)))
				to_chat(user, "<span class='warning'>The [abbreviated_name]'s error light flickers!</span>")
				return 0
