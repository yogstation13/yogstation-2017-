...
/*
 *Stunchucks - the security equivalent of a double bladed esword.
 */
/obj/item/weapon/twohanded/stunchucks
	name = "stunchucks"
	desc = "unleash your kung-fury, and show the syndicates who's the coolest."
	icon_state = "stunchucks"
	item_state = "stunchucks"
	...
	origin_tech = "combat=4" // double the batons double the research
	attack_verb = list("attacked", "beaten", "whacked",)
	block_chance = 60
	var/stunforce = 10
	var/obj/item/weapon/stock_parts/cell/high/bcell = null
	var/hitcost = 500 // twice the baton so only half the hitcost, since you have double the energy cell!

/obj/item/weapon/twohanded/stunchucks/suicide_act(mob/user)
	user.visible_message("<span class='suicide'>[user] starts doing a crazy trick with the [name] ! It looks like \he's trying to commit suicide.</span>")
	return (BRUTELOSS)

/obj/item/weapon/twohanded/stunchucks/New()
	..()
	update_icon()
	return

/obj/item/weapon/twohanded/stunchucks/loaded/New() //this one starts with a cell pre-installed.
	..()
	bcell = new(src)
	update_icon()
	return

/obj/item/weapon/twohanded/stunchucks/proc/deductcharge(chrgdeductamt)
	if(bcell)
...
/obj/item/weapon/twohanded/stunchucks/update_icon()
	if(wielded)
...
/obj/item/weapon/twohanded/stunchucks/examine(mob/user)
	..()
	if(bcell)
...
/obj/item/weapon/twohanded/stunchucks/attackby(obj/item/weapon/W, mob/user, params)
...
/obj/item/weapon/twohanded/stunchucks/attack_self(mob/user)
	if(bcell && bcell.charge > hitcost)
		wielded = !wielded
		user << "<span class='notice'>[src] is now [wielded ? "on" : "off"].</span>"
		playsound(loc, "sparks", 75, 1, -1)
	else
		wielded = 0
		if(!bcell)
...
/obj/item/weapon/twohanded/stunchucks/attack(mob/M, mob/living/carbon/human/user)
	if(wielded && user.disabilities & CLUMSY && prob(50))
		user.visible_message("<span class='danger'>[user] accidentally hits themself with [src]!</span>", \
							"<span class='userdanger'>You accidentally end up smacking yourself in the face with /The [src]!</span>")
...
/obj/item/weapon/twohanded/stunchucks/proc/baton_stun(mob/living/L, mob/user)
	if(ishuman(L))
...
/obj/item/weapon/twohanded/stunchucks/emp_act(severity)
	deductcharge(1000 / severity)
	..()

/obj/item/weapon/twohanded/stunchucks/hit_reaction(mob/living/carbon/human/owner, attack_text, final_block_chance)
	if(wielded)
		return ..()
	return 0
