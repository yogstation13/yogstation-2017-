...
/obj/item/proc/equipped(mob/user, slot)
	...
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
		if(anthrax_laced && ((slot != slot_l_hand && slot != slot_r_hand && (slot == slot_head || slot == slot_wear_mask || slot == slot_w_uniform || slot == slot_gloves || slot == slot_shoes || slot == slot_ear || slot == slot_eyes)) || (!H.gloves || istype(H.gloves, /obj/item/clothing/gloves/fingerless))))
