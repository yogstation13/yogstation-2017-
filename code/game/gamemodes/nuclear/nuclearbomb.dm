	...
	switch(deconstruction_state)
		...
		if(NUKESTATE_CORE_EXPOSED)
			...
			/*if(istype(I, /obj/item/toy/beach_ball/holoball))
				to_chat(user, "<span class='notice'>You touch the b-ball to the plutonium core...</span>")
				if(do_after(user, 100, target=src))
					qdel(I)
					qdel(core)
					var/obj/item/toy/beach_ball/holoball/chaos/bball = new /obj/item/toy/beach_ball/holoball/chaos
					user.put_in_hands(bball)
					deconstruction_state = NUKESTATE_CORE_REMOVED
					update_icon()
					to_chat(user, "<span class='notice'>The basketball absorbs the plutonium core and drops to the floor.</span>" )
				return*/
	return ..()
...
/obj/machinery/nuclearbomb/bananium/explode()
	...
	if(ticker.mode)
		...
		if(ticker.mode.name == "nuclear emergency")
			...
			to_chat(world, "<B>The station was honked by the bananium blast!</B>")
