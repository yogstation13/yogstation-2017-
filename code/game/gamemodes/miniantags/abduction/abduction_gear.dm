...
/obj/item/weapon/abductor_baton
	...
	var/stunstrength = 7
	var/sleepstrength = 60
	var/cuffstrength = 30 // this is used for time. the less the better
...
/obj/item/weapon/abductor_baton/proc/StunAttack(mob/living/L,mob/living/user)
	...
	L.Stun(stunstrength)
	L.Weaken(stunstrength)
	L.apply_effect(STUTTER, stunstrength)
...
/obj/item/weapon/abductor_baton/proc/CuffAttack(mob/living/L,mob/living/user)
	...
	if(!C.handcuffed)
		if(C.get_num_arms() >= 2)
			...
			if(do_mob(user, C, cuffstrength) && C.get_num_arms() >= 2)
				if(!C.handcuffed)
