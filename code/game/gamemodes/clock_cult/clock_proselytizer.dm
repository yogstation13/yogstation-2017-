...
/obj/item/clockwork/clockwork_proselytizer/examine(mob/living/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		...
		if(uses_alloy)
			to_chat(user, "<span class='alloy'>It has <b>[stored_alloy]/[max_alloy]</b> units of liquified alloy stored.</span>")
			...
			to_chat(user, "<span class='alloy'>Use it in-hand to remove stored liquified alloy.</span>")
...
/obj/item/clockwork/clockwork_proselytizer/attack_self(mob/living/user)
	if(is_servant_of_ratvar(user) && uses_alloy)
		if(!can_use_alloy(REPLICANT_ALLOY_UNIT))
			to_chat(user, "<span class='warning'>[src] [stored_alloy ? "Lacks enough":"Contains no"] alloy to reform[stored_alloy ? "":" any"] into solidified alloy!</span>")
			return
...
/obj/item/clockwork/clockwork_proselytizer/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/clockwork/component/replicant_alloy) && is_servant_of_ratvar(user) && uses_alloy)
		...
		to_chat(user, "<span class='brass'>You force [I] to liquify and pour it into [src]'s compartments. It now contains <b>[stored_alloy]/[max_alloy]</b> units of liquified alloy.</span>")
		user.drop_item()
		qdel(I)
		return 1
	else
...
/obj/item/clockwork/clockwork_proselytizer/proc/proselytize(atom/target, mob/living/user)
	...
	if(!can_use_alloy(proselytize_values["alloy_cost"]))
		if(stored_alloy - proselytize_values["alloy_cost"] < 0)
			to_chat(user, "<span class='warning'>You need <b>[proselytize_values["alloy_cost"]]</b> liquified alloy to proselytize [target]!</span>")
