...
/obj/item/clockwork/slab/proc/recite_scripture(mob/living/user)
	...
	switch(scripture_tier)
		if(SCRIPTURE_DRIVER,SCRIPTURE_SCRIPT,SCRIPTURE_APPLICATION,SCRIPTURE_REVENANT,SCRIPTURE_JUDGEMENT); //; for the empty if
			...
			to_chat(user, "<span class='warning'>That section of scripture is still locked!</span>")
			return 0
	...
	if(!ratvar_awakens && !no_cost && !tiers_of_scripture[scripture_to_recite.tier])
		to_chat(user, "<span class='warning'>That scripture is no longer unlocked, and cannot be recited!</span>")
		return 0
...
/obj/item/clockwork/slab/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		...
		to_chat(user, "Clockwork slabs will only generate components if held by a human or if inside a storage item held by a human, and when generating a component will prevent all other slabs held from generating components.<br>")
		to_chat(user, "Attacking a slab, a fellow Servant with a slab, or a cache with this slab will transfer this slab's components into that slab's components, their slab's components, or the global cache, respectively.")
		if(clockwork_caches)
			to_chat(user, "<b>Stored components (with global cache):</b>")
			for(var/i in stored_components)
				to_chat(user, "<span class='[get_component_span(i)]_small'><i>[get_component_name(i)]s:</i> <b>[stored_components[i]]</b> (<b>[stored_components[i] + clockwork_component_cache[i]]</b>)</span>")
		else
			to_chat(user, "<b>Stored components:</b>")
			for(var/i in stored_components)
				to_chat(user, "<span class='[get_component_span(i)]_small'><i>[get_component_name(i)]s:</i> <b>[stored_components[i]]</b></span>")
...
/obj/item/device/mmi/posibrain/soul_vessel/attack(mob/living/target, mob/living/carbon/human/user)
	...
	if(is_servant_of_ratvar(target))
		to_chat(user, "<span class='heavy_alloy'>\"It would be more wise to revive your allies, friend.\"</span>")
		return
	...
	if(!HE)
		...
		return
	if(H.stat == CONSCIOUS)
		to_chat(user, "<span class='warning'>[H] must be dead or unconscious to claim their mind!</span>")
		return
	if(H.head)
