...
/obj/structure/clockwork/powered/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		...
		if(z != ZLEVEL_STATION && z != ZLEVEL_CENTCOM || initial(area.name) == "Space")
			to_chat(user, "<span class='warning'>It cannot use power as is not near the station.</span>")
...
/obj/structure/clockwork/powered/mending_motor/examine(mob/user)
	..()
	if(is_servant_of_ratvar(user) || isobserver(user))
		to_chat(user, "<span class='alloy'>It contains <b>[stored_alloy*0.04]/[max_alloy*0.04]</b> units of liquified alloy, which is equivalent to <b>[stored_alloy]W/[max_alloy]W</b> of power.</span>")
		to_chat(user, "<span class='inathneq_small'>It requires <b>[mob_cost]W</b> to heal clockwork mobs, <b>[structure_cost]W</b> for clockwork structures, and <b>[cyborg_cost]W</b> for cyborgs.</span>")
...
/obj/structure/clockwork/powered/interdiction_lens/examine(mob/user)
	..()
	to_chat(user, "<span class='[recharging > world.time ? "nezbere_small":"brass"]'>Its gemstone [recharging > world.time ? "has been breached by writhing tendrils of blackness that cover the totem" \)
	: "vibrates in place and thrums with power"].</span>"
	if(is_servant_of_ratvar(user) || isobserver(user))
		to_chat(user, "<span class='nezbere_small'>It requires <b>[disrupt_cost]W</b> of power for each nearby disruptable electronic.</span>")
		to_chat(user, "<span class='nezbere_small'>If it fails to both drain any power and disrupt any electronics, it will disable itself for <b>[round(recharge_time/600, 1)]</b> minutes.</span>")
